...
/* 默认平台配置 */
static PlatformConfig _platform_config = {
...
bool observer_platform_init(void)
{
    ...
    // 复制配置
    memcpy(&_platform_config, &config, sizeof(PlatformConfig));
...
{
    return _platform_config.malloc ? _platform_config.malloc(size) : malloc(size);
}
...
void observer_platform_deinit(void)
{
    ...
    memset(&_platform_config, 0, sizeof(PlatformConfig));
}
